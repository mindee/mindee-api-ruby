      ...
      module Payslip
        ...
        class PayslipV3PaidTimeOff < Mindee::Parsing::Standard::FeatureField
          ...
          def printable_values
            printable = {}
            printable[:accrued] = @accrued.nil? ? '' : BaseField.float_to_string(@accrued)
            printable[:period] = format_for_display(@period)
            printable[:pto_type] = format_for_display(@pto_type)
            printable[:remaining] = @remaining.nil? ? '' : BaseField.float_to_string(@remaining)
            printable[:used] = @used.nil? ? '' : BaseField.float_to_string(@used)
            printable
          end
