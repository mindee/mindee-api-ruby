...
module Mindee
  module Prediction
    ...
    class ProofOfAddressV1 < Prediction
      ...
      attr_reader :issuer_company_registrations
      ...
      # Generic: VAT NUMBER, TAX ID, COMPANY REGISTRATION NUMBER or country specific.
      # @return [Array<Mindee::CompanyRegistration>]
      attr_reader :recipient_company_registrations
      ...
      def initialize(prediction, page_id)
        ...
        prediction['issuer_company_registrations'].each do |item|
          @issuer_company_registrations.push(CompanyRegistration.new(item, page_id))
        end
        ...
        prediction['recipient_company_registrations'].each do |item|
          @recipient_company_registrations.push(CompanyRegistration.new(item, page_id))
        end
      end

      def to_s
        recipient_company_registrations = @recipient_company_registrations.join(' ')
        issuer_company_registrations = @issuer_company_registrations.join(' ')
        dates = @dates.join(' ')
        out_str = String.new
