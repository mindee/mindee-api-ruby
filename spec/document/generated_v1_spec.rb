...
RSpec.describe 'Invoice V4 document' do
  ...
  let(:invoice_v4_complete_page0) do
    parsed_file = JSON.parse(File.read(File.join(DATA_DIR,
                                                 'products',
                                                 'generated',
                                                 'response_v1',
                                                 'complete_invoice_v4.json')))
    Page.new(GeneratedV1Page,
             parsed_file['document']['inference']['pages'][0])
  end
  ...
  describe 'String representation' do
    ...
    it 'ensures all fields are populated correctly' do
      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_address"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_address"].value).to eq("1954 Bloon Street West Toronto, ON, M6P 3K9 Canada")

      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_company_registrations"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_company_registrations"].values.length).to eq(0)

      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_name"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["customer_name"].value).to eq("JIRO DOI")

      expect(invoice_v4_complete_doc.inference.prediction.fields["date"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["date"].value).to eq("2020-02-17")

      expect(invoice_v4_complete_doc.inference.prediction.fields["document_type"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["document_type"].value).to eq("INVOICE")

      expect(invoice_v4_complete_doc.inference.prediction.fields["due_date"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["due_date"].value).to eq("2020-02-17")

      expect(invoice_v4_complete_doc.inference.prediction.fields["invoice_number"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["invoice_number"].value).to eq("0042004801351")

      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0]).to be_an_instance_of(GeneratedObjectField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].description).to eq("S)BOIE 5X500 FEUILLES A4")
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].product_code).to be_nil
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].quantity).to be_nil
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[6].quantity).to eq("1.0")
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].tax_amount).to be_nil
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].tax_rate).to be_nil
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].total_amount).to eq("2.63")
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[0].unit_price).to be_nil
      expect(invoice_v4_complete_doc.inference.prediction.fields["line_items"].values[6].unit_price).to eq("65.0")

      expect(invoice_v4_complete_doc.inference.prediction.fields["locale"]).to be_an_instance_of(GeneratedObjectField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["locale"].currency).to eq("EUR")
      expect(invoice_v4_complete_doc.inference.prediction.fields["locale"].language).to eq("fr")

      expect(invoice_v4_complete_doc.inference.prediction.fields["reference_numbers"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["reference_numbers"].values[0].value).to eq("AD29094")

      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_address"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_address"].value).to eq("156 University Ave, Toronto ON, Canada M5H 2H7")
      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_company_registrations"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_company_registrations"].values.length).to eq(0)

      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_name"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_name"].value).to eq("TURNPIKE DESIGNS CO.")

      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_payment_details"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["supplier_payment_details"].values[0].iban).to eq("FR7640254025476501124705368")

      expect(invoice_v4_complete_doc.inference.prediction.fields["taxes"]).to be_an_instance_of(GeneratedListField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["taxes"].values[0].polygon).to be_an_instance_of(PositionField)
      expect(
        invoice_v4_complete_doc.inference.prediction.fields["taxes"].values[0].polygon.value.map { |point| [point.x, point.y] }
      ).to eq([[0.292, 0.749], [0.543, 0.749], [0.543, 0.763], [0.292, 0.763]])
      expect(invoice_v4_complete_doc.inference.prediction.fields["taxes"].values[0].rate).to eq("20.0")
      expect(invoice_v4_complete_doc.inference.prediction.fields["taxes"].values[0].value).to eq("97.98")

      expect(invoice_v4_complete_doc.inference.prediction.fields["total_amount"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["total_amount"].value).to eq("587.95")

      expect(invoice_v4_complete_doc.inference.prediction.fields["total_net"]).to be_an_instance_of(StringField)
      expect(invoice_v4_complete_doc.inference.prediction.fields["total_net"].value).to eq("489.97")
