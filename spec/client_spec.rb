...
describe Mindee::Client do
  context 'A client' do
    ...
    it 'should open PDF files from a path' do
      input_source = mindee_client.source_from_path("#{DATA_DIR}/invoice/invoice.pdf")
      expect(input_source).to respond_to(:read_document)
      input_source = mindee_client.source_from_path("#{DATA_DIR}/invoice/invoice_10p.pdf")
      expect(input_source).to respond_to(:read_document)
    end
    ...
    it 'should open JPG files from a path' do
      input_source = mindee_client.source_from_path("#{DATA_DIR}/receipt/receipt.jpg")
      expect(input_source).to respond_to(:read_document)
      input_source = mindee_client.source_from_path("#{DATA_DIR}/receipt/receipt.jpga")
      expect(input_source).to respond_to(:read_document)
    end
    ...
    it 'should make an invalid API async enqueue call raising an exception' do
      ...
      input_source = mindee_client1.source_from_file(file, '2_invoices.pdf')
